...
exports.guardarDatos = async (req, res) => {
  try {
    ...
    // Manejar datos del segundo formulario (preguntas)
      ...
      const respuestasEmpatadas = [];
      let segundoMaxRepeticiones = 0;
      const umbralMinimo = 2; // Umbral mínimo de votos para la segunda carrera

      for (const respuesta in conteoRespuestas) {
        if (conteoRespuestas[respuesta] > segundoMaxRepeticiones && conteoRespuestas[respuesta] < maxRepeticiones) {
          segundoMaxRepeticiones = conteoRespuestas[respuesta];
          respuestasEmpatadas.length = 0; // Reiniciar el array de respuestas empatadas
          respuestasEmpatadas.push(respuesta);
        } else if (conteoRespuestas[respuesta] === segundoMaxRepeticiones && conteoRespuestas[respuesta] >= umbralMinimo) {
          respuestasEmpatadas.push(respuesta);
        }
      }

      // Elegir aleatoriamente una respuesta en caso de empate
      let segundaRespuestaMasRepetida = '';
      if (respuestasEmpatadas.length > 0) {
        const indiceAleatorio = Math.floor(Math.random() * respuestasEmpatadas.length);
        segundaRespuestaMasRepetida = respuestasEmpatadas[indiceAleatorio];
      }

      // Obtener la segunda carrera correspondiente
      const segundaCarreraElegida = carreraCorrespondiente(parseInt(segundaRespuestaMasRepetida));
      ...
      await estudiante.update({ carreraElegida, segundaCarreraElegida });
